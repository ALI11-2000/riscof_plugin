...
class swerv1(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","swerv1")
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       self.hexgen_cmd = 'riscv64-unknown-elf-objcopy -O verilog  my.elf my.hex'
        
        # runs only once
       build_swerv1 = 'cd $RV_ROOT \n \
       make -f ./tools/Makefile'

    #    utils.shellCommand(build_swerv1).run()   
       # verilator run test
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for swerv1 start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
...
# The following template only uses shell commands to compile and run the tests.
    ...
    def runTests(self, testList):
        ...
        for testname in testList:
            ...
            utils.shellCommand(cmd1).run(cwd=test_dir)
            ...
            sigdump_run = self.sigdump_cmd.format(sig_file,test_dir)
            utils.shellCommand(sigdump_run).run(cwd=test_dir)
